        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden transition-all hover:border-border/80">
            ...
            <div className="p-4 sm:p-5">
                <div className="flex items-start gap-3">
                    <div className={`p-2.5 rounded-xl ${isFullyPaid ? 'bg-green-50 dark:bg-green-950/40 text-green-600' : item.is_access_suspended ? 'bg-red-50 dark:bg-red-950/40 text-red-600' : 'bg-primary/10 text-primary'} flex-shrink-0 mt-0.5`}>
                        {isFullyPaid
                ...
                <div className="mt-4">
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all ${isFullyPaid ? 'bg-green-500' : item.is_access_suspended ? 'bg-red-500' : 'bg-primary'}`}
...
export default function Installments({ installments }: InstallmentsProps) {
    return (
        <UserLayout>
            <Head title="Cicilan Saya - Talenta Edu" />
            <ProfileLayout>
                <div className="space-y-6">
                    <Heading
                        title="Cicilan Saya"
                        description="Kelola semua pembayaran cicilan program pembelajaran yang aktif"
                    />
                    ...
                    {installments.length === 0 ? (
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/20 py-16 text-center">
                            ...
                            <p className="text-sm text-muted-foreground max-w-xs">
                                Anda belum memiliki pembayaran cicilan aktif. Pilih program yang mendukung cicilan untuk mulai.
                            </p>
                        </div>
                    ) : (
